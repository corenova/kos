...
(function (global){
"use strict";function _toConsumableArray(r){if(Array.isArray(r)){for(var e=0,t=Array(r.length);e<r.length;e++)t[e]=r[e];return t}return Array.from(r)}function syncConnect(r){this.send("link/connect/url",r)}function syncListen(r){this.send("link/listen/url",r)}function syncReactor(r){this.send("sync",r)}function syncStream(stream){var _this=this,_stream$state$get=stream.state.get("link"),addr=_stream$state$get.addr,opts=_stream$state$get.opts,reactor=kos.create({name:"sync("+addr+")",purpose:"reactions to sync with remote peer",passive:!0,enabled:!1});reactor.on("data",function(r){if(r.origin!==reactor.id&&!reactor.active){var e=kos.reactors.filter(function(r){return!r.passive}).reduce(function(r,e){return r.concat(e.inputs)},[]);r.match(e.concat(["module/*"]))||stream.write(r)}}),stream.on("data",function(r){var e=r.key,t=r.value;if("sync"!==e)return reactor.write(r);if(!(t instanceof kos.Reactor)){if("string"==typeof t)return void(t!==reactor.name&&(_this.info("discovered own reactor name from peer",t),reactor.state.set("remote",t)));if(t.id===kos.id)return kos.warn("detected circular sync loop from",addr),void kos.unload(reactor);if(_this.info("importing '"+t.name+"' reactor ("+t.id+") from "+addr),"kos"===t.name){var n=t.reactors.filter(function(r){return r.name!==reactor.state.get("remote")&&!kos.reactors.some(function(e){return r.name===e.name})});_this.debug("unload existing reactors...",reactor.reactors.map(function(r){return r.name})),reactor.reactors.forEach(function(r){return reactor.unload(r)}),reactor.load.apply(reactor,_toConsumableArray(n)),_this.debug("imported "+n.length+" new reactor(s):",n.map(function(r){return r.name}))}else{if(kos.reactors.some(function(r){return r.name===t.name}))return;reactor.load(t)}reactor.send("sync",reactor)}}),stream.pause(),stream.on("active",function(){reactor.active&&reactor.disable(),stream.feed("sync",reactor.name).feed("sync",kos),stream.resume(),_this.debug("synchronizing with:",addr)}),stream.on("inactive",function(){function restore(target){target.triggers.forEach(function(x){if(!(x.handler instanceof Function)){reactor.info("restoring",x.name);try{x._handler=eval("("+x.handler.source+")")}catch(r){reactor.error("unable to restore handler from source",x.handler.source,r)}}}),target.reactors.forEach(restore)}opts.persist&&(_this.info("maintaining capabilities from",reactor.name),restore(reactor),reactor.enable(),reactor.send("reactor",reactor))}),stream.on("destroy",function(){_this.debug("destroying sync stream, unload:",reactor.name),kos.unload(reactor),stream.end(),_this.send("sync",kos)}),kos._load(reactor)}var _global=global,_global$kos=_global.kos,kos=void 0===_global$kos?require(".."):_global$kos,link=require("./link");module.exports=kos.create("sync").desc("reactions to synchronize dataflow objects with remote stream(s)").load(link).in("sync/connect").out("link/connect/url").bind(syncConnect).in("sync/listen").out("link/listen/url").bind(syncListen).in("link/stream").out("sync").bind(syncStream).in("reactor").out("sync").bind(syncReactor);
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"..":undefined,"./link":45}],50:[function(require,module,exports){
(function (global){
"use strict";function connect(e){var s=this,t=this.get("module/simple-websocket"),n=this.get("protocols"),o=this.get("links"),r=e=normalizeOptions(e),i=r.protocol,c=r.hostname,l=r.port,a=r.path,u=r.retry,d=r.max;if(!n.includes(i))return this.error("unsupported protocol",i);var p=i+"//"+c+":"+l+"/"+a;if(o.has(p))return this.send("link",o.get(p));var h=new t(p),m={addr:p,socket:h,opts:e};o.set(p,m),this.send("link",m),h.on("connect",function(){s.info("connected to",p),s.send("ws/socket",h),h.emit("active"),u&&(u=100)}),h.on("close",function(){u&&setTimeout(function(){e=Object.assign({},e,{retry:Math.round(Math.min(d,1.5*u))}),s.debug("attempt reconnect",p),o.delete(p),s.feed("ws/connect",e)},u)}),h.on("error",this.error.bind(this))}function listen(e){var s=this,t=this.get("module/simple-websocket/server"),n=this.get("protocols"),o=e=normalizeOptions(e),r=o.protocol,i=o.hostname,c=o.port,l=o.path,a=o.server;if(!n.includes(r))return this.error("unsupported protocol",r);this.debug("attempt",i,c),a?(a=new t({server:a}),this.info("listening on existing server instance"),this.send("ws/server",a)):(a=new t({host:i,port:c,path:l}),this.info("listening",i,c,l),this.send("ws/server",a)),a.on("connection",function(t){var n=t._ws._socket,o=r+"//"+n.remoteAddress+":"+n.remotePort;s.info("accept",o),s.send("ws/socket",t),s.send("link",{addr:o,socket:t,server:a,opts:e}),t.emit("active")}),a.on("error",this.error.bind(this))}function connectByUrl(e){var s=this.get("module/url"),t=s.parse(e,!0);t.slashes||(t=s.parse("ws://"+e,!0)),this.send("ws/connect",Object.assign(t,t.query))}function listenByUrl(e){var s=this.get("module/url"),t=s.parse(e,!0);t.slashes||(t=s.parse("ws://"+e,!0)),this.send("ws/listen",Object.assign(t,t.query))}function normalizeOptions(e){return{protocol:e.protocol||"ws:",hostname:e.hostname||"0.0.0.0",port:parseInt(e.port,10)||80,path:e.path||"",server:e.server,retry:parseInt(e.retry,10)||100,max:parseInt(e.max,10)||5e3,persist:"persist"in e}}var _global=global,_global$kos=_global.kos,kos=void 0===_global$kos?require(".."):_global$kos;module.exports=kos.create("ws").desc("reactions to establish WebSocket client/server communication links").init("protocols",["ws:","wss:"]).init("links",new Map).in("ws/connect").and.has("module/simple-websocket").out("ws/socket","link","ws/connect").bind(connect).in("ws/listen").and.has("module/simple-websocket/server").out("ws/server","ws/socket","link").bind(listen).in("ws/connect/url").and.has("module/url").out("ws/connect").bind(connectByUrl).in("ws/listen/url").and.has("module/url").out("ws/listen").bind(listenByUrl);
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
