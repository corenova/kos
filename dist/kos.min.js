...
(function (global){
"use strict";function simpleGet(e){this.send("http/request",{url:e,method:"GET"})}function clientRequest(e){var t=this,r=this.get("module/superagent"),s=e.url,n=e.method,o=e.data;switch(n=n.toLowerCase()){case"get":case"delete":r[n](s).end(function(e,r){e?t.throw(e):t.send("http/response",r)});break;case"post":case"put":case"patch":r[n](s).send(o).end(function(e,r){e?t.throw(e):t.send("http/response",r)})}}function createServer(e){var t=this,r=this.get("module/http","module/url"),s=_slicedToArray(r,2),n=s[0],o=s[1];if("string"==typeof e){var i=e;e=o.parse(i,!0),e.slashes||(e=o.parse("http://"+i,!0))}var a=e.protocol||"http:",u=e.hostname||"0.0.0.0",h=parseInt(e.port,10)||80,l=n.createServer(function(e,r){t.send("http/server/request",{req:e,res:r})});l.on("connection",function(e){var r=a+"//"+e.remoteAddress+":"+e.remotePort;t.info("accept",r),t.send("http/socket",e),t.send("http/link",{addr:r,socket:e}),e.emit("active")}),l.on("listening",function(){t.info("listening",u,h),t.send("http/server",l)}),l.on("error",this.error.bind(this)),this.debug("attempt",u,h),l.listen(h,u)}function classifyServerTransaction(e){var t=e.req,r=(e.res,t.method.toLowerCase());this.send("http/server/request/"+r,e)}function handleRoute(e,t){}function proxy(e,t){this.send("http/request",Object.assign({},e,{host:t.host}))}var _slicedToArray=function(){function e(e,t){var r=[],s=!0,n=!1,o=void 0;try{for(var i,a=e[Symbol.iterator]();!(s=(i=a.next()).done)&&(r.push(i.value),!t||r.length!==t);s=!0);}catch(e){n=!0,o=e}finally{try{!s&&a.return&&a.return()}finally{if(n)throw o}}return r}return function(t,r){if(Array.isArray(t))return t;if(Symbol.iterator in Object(t))return e(t,r);throw new TypeError("Invalid attempt to destructure non-iterable instance")}}(),_global=global,_global$kos=_global.kos,kos=void 0===_global$kos?require(".."):_global$kos;module.exports=kos.create("http").desc("reactions to HTTP client/server requests").in("http/request").and.has("module/superagent").out("http/response").bind(clientRequest).in("http/request/get").out("http/request").bind(simpleGet).in("http/listen").and.has("module/http","module/url").out("http/server","http/socket","link","http/server/request").bind(createServer).in("http/server/request").out("http/server/request/*").bind(classifyServerTransaction).in("http/server","http/route").out("http/server/request").bind(handleRoute);
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
...
(function (global){
"use strict";function _toConsumableArray(n){if(Array.isArray(n)){for(var e=0,r=Array(n.length);e<n.length;e++)r[e]=n[e];return r}return Array.from(n)}function syncConnect(n){this.send("link/connect/url",n)}function syncListen(n){this.send("link/listen/url",n)}function syncReactor(n){this.send("sync",n)}function syncStream(n){var e=this,r=n.state.get("link"),o=r.addr,t=r.opts,i=kos.create({name:"sync("+o+")",purpose:"reactions to sync with remote peer",passive:!0,enabled:!1});i.on("data",function(e){if(e.origin!==i.id&&!i.active){var r=kos.reactors.filter(function(n){return!n.passive}).reduce(function(n,e){return n.concat(e.inputs)},[]);e.match(r.concat(["module/*"]))||n.write(e)}}),n.on("data",function(n){var r=n.key,t=n.value;if("sync"!==r)return i.write(n);if(!(t instanceof kos.Reactor)){if("string"==typeof t)return void(t!==i.name&&(e.info("discovered own reactor name from peer",t),i.state.set("remote",t)));if(t.id===kos.id)return kos.warn("detected circular sync loop from",o),void kos.unload(i);if(e.info("importing '"+t.name+"' reactor ("+t.id+") from "+o),"kos"===t.name){var a=t.reactors.filter(function(n){return n.name!==i.state.get("remote")&&!kos.reactors.some(function(e){return n.name===e.name})});e.debug("unload existing reactors...",i.reactors.map(function(n){return n.name})),i.reactors.forEach(function(n){return i.unload(n)}),i.load.apply(i,_toConsumableArray(a)),e.debug("imported "+a.length+" new reactor(s):",a.map(function(n){return n.name}))}else{if(kos.reactors.some(function(n){return n.name===t.name}))return;i.load(t)}i.send("sync",i)}}),n.pause(),n.on("active",function(){i.active&&i.disable(),n.feed("sync",i.name).feed("sync",kos),n.resume(),e.debug("synchronizing with:",o)}),n.on("inactive",function(){function n(e){e.triggers.forEach(function(n){if(!(n.handler instanceof Function)){i.info("restoring "+e.identity+":"+n.name);try{n._handler=new Function("return ("+n.handler.source+")")()}catch(e){i.error("unable to restore handler from source",n.handler.source,e)}}}),e.reactors.forEach(n)}t.persist&&(e.info("maintaining capabilities from",i.name),n(i),i.enable(),i.send("reactor",i))}),n.on("destroy",function(){e.debug("destroying sync stream, unload:",i.name),kos.unload(i),n.end(),e.send("sync",kos)}),kos._load(i)}var _global=global,_global$kos=_global.kos,kos=void 0===_global$kos?require(".."):_global$kos,link=require("./link");module.exports=kos.create("sync").desc("reactions to synchronize dataflow objects with remote stream(s)").load(link).in("sync/connect").out("link/connect/url").bind(syncConnect).in("sync/listen").out("link/listen/url").bind(syncListen).in("link/stream").out("sync").bind(syncStream).in("reactor").out("sync").bind(syncReactor);
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
