...
(function (global){
"use strict";function syncConnect(n){this.send("link/connect/url",n)}function syncListen(n){this.send("link/listen/url",n)}function syncStream(n){var e=this;n.feed("reactors",kos.reactors),this.parent.pipe(n),n.on("active",function(){n.pipe(e.parent),e.debug("synchronizing")})}var _global=global,_global$kos=_global.kos,kos=void 0===_global$kos?require(".."):_global$kos,link=require("./link");module.exports=kos.create("sync").desc("reactions to synchronize dataflow objects with remote stream(s)").load(link).in("sync/connect").out("link/connect/url").bind(syncConnect).in("sync/listen").out("link/listen/url").bind(syncListen).in("link/stream").bind(syncStream);
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"..":undefined,"./link":45}],50:[function(require,module,exports){
(function (global){
"use strict";function connect(e){var t=this,s=this.get("module/simple-websocket"),n=this.get("protocols"),o=this.get("links"),r=normalizeOptions(e),i=r.protocol,c=r.hostname,l=r.port,a=r.path,u=r.retry,d=r.max;if(!n.includes(i))return this.error("unsupported protocol",i);var h=i+"//"+c+":"+l+"/"+a;if(o.has(h))return this.send("link",o.get(h));var p=new s(h),m={addr:h,socket:p};o.set(h,m),this.send("link",m),p.on("connect",function(){t.info("connected to",h),t.send("ws/socket",p),p.emit("active"),u&&(u=100)}),p.on("close",function(){u&&setTimeout(function(){t.debug("attempt reconnect",h),o.delete(h),t.send("ws/connect",Object.assign({},e,{retry:Math.round(Math.min(d,1.5*u))}),{id:null})},u)}),p.on("error",this.error.bind(this))}function listen(e){var t=this,s=this.get("module/simple-websocket/server"),n=this.get("protocols"),o=normalizeOptions(e),r=o.protocol,i=o.hostname,c=o.port,l=o.path,a=o.server;if(!n.includes(r))return this.error("unsupported protocol",r);this.debug("attempt",i,c),a?(a=new s({server:a}),this.info("listening on existing server instance"),this.send("ws/server",a)):(a=new s({host:i,port:c,path:l}),this.info("listening",i,c,l),this.send("ws/server",a)),a.on("connection",function(e){var s=e._ws._socket,n=r+"//"+s.remoteAddress+":"+s.remotePort;t.info("accept",n),t.send("ws/socket",e),t.send("link",{addr:n,socket:e}),e.emit("active")}),a.on("error",this.error.bind(this))}function connectByUrl(e){var t=this.get("module/url"),s=t.parse(e,!0);s.slashes||(s=t.parse("ws://"+e,!0)),this.send("ws/connect",Object.assign(s,s.query))}function listenByUrl(e){var t=this.get("module/url"),s=t.parse(e,!0);s.slashes||(s=t.parse("ws://"+e,!0)),this.send("ws/listen",Object.assign(s,s.query))}function normalizeOptions(e){return{protocol:e.protocol||"ws:",hostname:e.hostname||"0.0.0.0",port:parseInt(e.port,10)||80,path:e.path||"",server:e.server,retry:parseInt(e.retry,10)||100,max:parseInt(e.max,10)||5e3}}var _global=global,_global$kos=_global.kos,kos=void 0===_global$kos?require(".."):_global$kos;module.exports=kos.create("ws").desc("reactions to establish WebSocket client/server communication links").init("protocols",["ws:","wss:"]).init("links",new Map).in("ws/connect").and.has("module/simple-websocket").out("ws/socket","link","ws/connect").bind(connect).in("ws/listen").and.has("module/simple-websocket/server").out("ws/server","ws/socket","link").bind(listen).in("ws/connect/url").and.has("module/url").out("ws/connect").bind(connectByUrl).in("ws/listen/url").and.has("module/url").out("ws/listen").bind(listenByUrl);
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
