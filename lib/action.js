...
class KineticAction extends KineticTransform {
  ...
  constructor(options={}) {
    ...
    let { inputs = [], outputs = [], handler = KineticAction.none } = options
  ...
  pipe(stream) {
    if (stream instanceof KineticAction) {
      if (!this.isChainable(stream))
        throw new Error("[KineticAction:pipe] cannot pipe to an incompatible stream")
