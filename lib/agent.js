...
class Agent extends Interface {
  ...
  set schema(x) {
    ...
    super.schema = x;
    const store = new Store(this.name).use(x);
    store
  ...
  use(schema) {
    ...
    return new Interface(schema).attach(this);
  }
  set() {
  ...
  propagate(event, prop, actor) {
    let { kind, path, content, change } = prop;
    if (kind !== 'module') {
      const pulse = new Pulse(`${path}`, prop).tag(this).tag(actor);
      switch (event) {
      case 'change':
        debug(`${this.uri} <<< ${path} (${kind})`);
        this.inflow.push(pulse.add(content));  // we push here to bypass "incoming" filter
        break;
      case 'update':
        debug(`${this.uri} >>> ${path} (${kind})`);
        this.outflow.push(pulse.add(change)); // we push here to bypass "outgoing" filter
        break;
      }
    }
  }
