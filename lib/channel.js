...
class Channel extends Duplex {
  ...
  constructor(source) {
    ...
    source.on('error', () => this.disconnect()); // handle when source stream emits error
    this.on('close', () => source.off('data', absorb));
    this.on('error', () => this.disconnect()); // handle when write to source has an error
  }
  connect(parent) {
  ...
  _write(pulse, encoding, callback) {
    ...
    if (pulse instanceof Pulse) {
      if (!pulse.tagged(this) && pulse.node && this[kParent]) {
        //let data = pulse.node ? pulse.toString(diff) : pulse.toString();
	let data = pulse.toString();
        debug(`${this.uri} [remote] <-- ${pulse.topic}`);
        debug(data);
