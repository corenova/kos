...
const Context = {
  ...
  save(state={}) {
    if (!this.state) {
      this.debug(`initialized with new state (${Object.keys(state)})`);
      this.state = state;
      this.emit('save', state);
    } else {
      const prop = this.state.$('.');
      prop.merge(state, { suppress: true })
      if (prop.changed)
  ...
  save2(state) {
    ...
    if (typeof state === "object") {
      ...
      debug(`${this} saving new state for: ${keys}`)
      for (let k of keys) {
        if (!equal(prev[k], state[k])) {
          ...
          try {
            ...
            this.error(`${this} unable to merge data for state key ${k}`, e)
          }
        }
      }
      if (Object.keys(change).length) {
        debug(`${this} changed for: ${Object.keys(change)}`)
  ...
  log(topic, ...args) {
    ...
    debug.enabled(topic) && root.emit('log', topic, args, actor);
  }
}
