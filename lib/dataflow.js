...
class Dataflow extends Transform {
  ...
  join(flow) {
    ...
    this.on('log',   this.emit.bind(flow,'log')) // propagate upstream
  ...
  error(err, ...rest) {
    ...
    err.origin = this
    this.push(new Stimulus('error', this.id).add(err))
    return err
  }
  ...
  io(opts = {}) {
    ...
    const wrapper = new Dataflow({
      ...
      transform: (chunk, encoding, done) => {
        if (chunk instanceof Stimulus) {
          if (chunk.origin && this.seen(chunk))
            wrapper.push(chunk.toKSON() + "\n")
        } else {
          buffer += chunk
          ...
          for (let line of lines.filter(Boolean)) {
            try { this.write(Stimulus.fromKSON(line.trim())) }
