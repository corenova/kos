...
class KineticEssence extends Transform {
  ...
  // A special "wrapper" method that creates a new Duplex stream
  // around the current stream instance. It provides compatibility
  // interface with non-KOS streams by transforming incoming/outgoing
  // chunks to/from KineticObjects.  
  //
  ...
  io(opts) {
    if (typeof opts === 'string') {
      opts = { input: opts, output: opts }
    }
    let { input='json', output='json', passthrough=false } = opts
    let id = uuid()
    let buffer = ''
    let lines
        ...
        case 'json':
          ...
          for (let line of lines.filter(Boolean))
            try {
              ...
              for (let k in obj)
                this.write(new KineticObject(k, obj[k], id))
            } catch (e) {
              debug("io:error", e)
              this.throw(e)
            }
    ...
    this.on('data', ko => {
      let str 
      if (!passthrough && ko.tags.has(id)) return
      if (/^module\//.test(ko.key) || ko.key === 'kos') return
      switch (output) {
      case 'json': 
        try {
