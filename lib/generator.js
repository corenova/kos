...
class Generator extends Neural.Layer {
  ...
  get inputs()  { return this.nodes.filter(n => n !== this.core) }
  get outputs() { return this.nodes.filter(n => n !== this.core) }
  
  // get inputs()  { return this.nodes.filter(n => Array.from(n.consumes.keys()).some(s => s.node)) }
  // get outputs() { return this.nodes.filter(n => Array.from(n.produces.keys()).some(s => s.node)) }
  ...
  set schema(x) {
    ...
    if (!(x.kind === 'kos:generator'))
      throw this.error(`Generator schema must be a Yang kos:generator, not ${x.kind}`);
    
    this[kSchema] = x;
    x.apply(this);
    this[kCore] = this.nodes.find(n => n.tag === 'core')
    this[kProp] = new Yang.Property(this.name, x)
  ...
  join(parent) {
    if (parent instanceof Neural.Layer) {
      this[kProp].container = parent
      return super.join(parent)
    }
    return parent
  }
  inspect() {
    const { id, type, name, uri, summary, consumes, produces } = this
    return {
