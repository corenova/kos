...
class Node extends Neural.Node {
  ...
  get flow() { return this[kFlow]; }
  get filter() { return false; }
  get ready() { return this.enabled; }
  get enabled() { return Array.from(this.depends).every(d => this.flow.has(d)); }
  get depends() {
  ...
  set depends(x=[]) {
    ...
    for (let feature of features)
      feature.binding && this.flow.set(feature, feature.binding)
  ...
  use(key) {
    ...
    let feature = this.flow.get(this.lookup('feature', key))
    if (!feature)
  ...
  activate(pulse, done) {
    ...
    //   this.flow.delete(schema)
    //   return false 
    // }
    if (this.depends.has(schema)) {
      this.flow.set(schema, data)
      return false
    }
