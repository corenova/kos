...
class Persona extends Neural.Layer {
  ...
  get consumes() {
    if (!super.consumes.size && this.reactors.length) {
  ...
  get produces() {
    if (!super.produces.size && this.reactors.length) {
  ...
  incoming(pulse) {
    ...
    if (topic === 'kos:runtime') {
      const { store } = data;
      try { this.model = store.access(this.tag) }
      catch (err) { throw this.error(err) }
      return true;
    }
    const allow = schema && schema.node && this.consumes.has(schema);
    allow && debug(`${this.uri} <<- ${topic} (${schema.kind})`);
    return allow;
  }
  outgoing(pulse) {
    const { topic, schema, data } = pulse;
    const allow = schema && schema.node && this.produces.has(schema);
    allow && debug(`${this.uri} ->> ${topic} (${schema.kind})`);
    return allow;
  }
  toJSON() {
