...
class Persona extends Dataflow {
  ...
  load(flow, state) {
    let foo = this.create(flow).save(state, { feed: true }).join(this)
    return this
  }
  unload(flow) {
        ...
        filter: stimulus => {
          ...
          if (this.seen(stimulus)) {
            ...
            if (stimulus.match(this.inputs.concat(this.requires))) {
              ...
              if (topic !== 'error') this.emit('ignore', stimulus)
              return false
            }
          } else { 
            // from internal flow
  ...
  get logs() {
    const logs = new Dataflow
    this.on('log', logs.write.bind(logs))
    return logs
  }
}
