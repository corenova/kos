...
class Persona extends Interface {
  ...
  get consumes() {
    if (!super.consumes.size) {
      let consumes = this.interfaces.map(i => Array.from(i.consumes))
  ...
  get produces() {
    if (!super.produces.size) {
      let produces = this.interfaces.map(i => Array.from(i.produces))
  ...
  constructor(props={}) {
    debug('constructing new persona', props);
    super(Object.assign(props, { active: true }));
  }
  at(uri) {
    const schema = this.locate(uri);
    let match = this.interfaces.find(x => x.schema === schema);
    if (!match) {
  ...
  incoming(pulse) {
    ...
    return (origin === this) || super.incoming(pulse);
  }
  outgoing(pulse) {
    const { schema } = pulse;
    return schema.node || super.outgoing(pulse);
  }
  toJSON() {
