...
class Reaction extends Node {
  ...
  get ready() {
    return this.active && Array.from(this.consumes.keys()).every(x => this.flow.has(x));
  }
  get context() {
  ...
  activate(pulse, done) {
    ...
    const args = inputs.map(k => this.flow.get(k).data).concat(context);
    ...
    context.inputs = inputs.map(k => this.flow.get(k));
    ...
    for (let [schema, opts] of consumes)
      opts.persist || this.flow.delete(schema)
