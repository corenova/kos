...
class Reaction extends DynamicFunction {
  ...
  constructor(props={}) {
    super(stimulus => this.exec(stimulus))
  ...
  log(type, ...args) {
    try { this.push(new Stimulus(type, this.id).add([ this.identity, ...args ])) }
    catch (e) {
  ...
  set()    { this.state.set(...arguments) }
  delete() { this.state.delete(...arguments) } 
  clear(...keys) {
    if (!arguments.length)
