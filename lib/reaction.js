...
class Reaction extends DynamicFunction {
  ...
  exec(stimulus) {
    if (this.ready(stimulus)) {
      ...
      for (let data of stimulus) {
        ...
        catch (e) { debug(e); this.log('error', e) }
      }
      ctx.flush()
      return true
    }
  ...
  ready(stimulus) {
    if (!(stimulus instanceof Stimulus)) return false
  ...
  push(stimulus) {
    if (!(stimulus instanceof Stimulus))
      throw new Error("[Reaction:push] must push an instance of Stimulus")
    for (let subflow of this._subflows) subflow.write(stimulus)
  }

  // logs from reaction gets emitted by its upstream flow
  log(type, ...args) { this.flow.emit('log', new Stimulus(type, this.id).add(...args)) }
  error() {
    this.log('error', ...arguments)
    return this.flow.error.apply(this, arguments)
  }
