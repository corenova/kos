  ...
  constructor(options) {
    ...
    this.core = new KineticStream({
      ...
      filter: (ko) => {
        ...
        if (this.seen(ko)) {
          // from external flow
          if (ko.match(['error','warn','info','debug'])) return false
          ...
          // from internal flow
          if (ko.match(['error','warn','info','debug'])) return true

          flow.push('feedback')
          let { inputs, absorbs, outputs } = this
          //outputs.push(...[ 'error', 'warn', 'info', 'debug' ])

          ko.match(inputs) ? flow.push('consume') : flow.push('reject')
