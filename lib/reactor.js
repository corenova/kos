    ...
    this.core = new KineticStream({
      ...
      filter: (token) => {
        ...
        if (this.seen(token)) {
          ...
          flow.push('accept')
          //this.emit('accept', token)
          let { requires, inputs, absorbs } = this
          if (token.match(requires) || token.match(inputs)) {
            debug(this.identity, '<==', token.key)
            // update the token that it's been accepted into this flow
            this.mark(token, true)
            flow.push('consume')
            if (token.match(absorbs))
              flow.push('absorb')
          } else {
            flow.push('reject')
            return false
          ...
          flow.push('feedback')
          let { inputs, absorbs, outputs } = this
          //outputs.push(...[ 'error', 'warn', 'info', 'debug' ])

          token.match(inputs) ? flow.push('consume') : flow.push('reject')

          if (token.match(absorbs)) {
            debug(this.identity, '<->', token.key)
            this.mark(token, true) // prevent external propagation
            flow.push('absorb')
          } else if (token.match(outputs)) {
            debug(this.identity, '==>', token.key)
            flow.push('produce')
          } else {
            debug(this.identity, '<--', token.key)
            this.mark(token) // prevent external propagataion
          }
          if (token.key !== 'error') 
