...
class KineticReactor extends KineticStream {
  ...
  pre(...keys) { return this.trigger({ requires: keys }) }
  in(...keys)  { return this.trigger({ inputs: keys }) }
  ...
  trigger(opts={}) { return new KineticTrigger(opts).join(this) }
  ...
  get notify()  { return this.listenerCount('flow') }
  ...
  get cache() {
    if (!this.state.has('cache')) {
      this.state.set('cache', {
        requires: (() => {
          let streams = this.triggers.concat(this.reactors)
      ...
      this.once('dirty', stream => this.state.delete('cache'))
    }
    return this.state.get('cache')
  }
  get requires() { return this.cache.requires }
  get inputs()   { return this.cache.inputs }
  get outputs()  { return this.cache.outputs }
  get consumes() { return this.cache.consumes }
  get absorbs()  { return this.cache.absorbs }
