...
class StateMachine extends Dataflow {
  ...
  get model()  { return this.prop('model') || this.prop('model', (
    this.schema ? this.schema.eval() : new Map
  ))}
  ...
  set state(x={}) {
    ...
    if (this.schema) this.model.set(x)
    else {
      for (let k in x) this.model.set(k,x[k])
    }
  }
  get state()  {
    if (this.schema) this.model.get()
    else {
      let obj = {}
      for (let [k,v] of this.model) obj[k] = v
      return obj
    }
  }

  set label(x)   { this.prop('label', x) }
  get label()    { return this.prop('label') || (this.schema ? this.schema.tag : this.id) }
  set purpose(x) { this.prop('purpose', x) }
  get purpose()  { return this.prop('purpose') || (this.schema ? this.schema.description : undefined) }
  ...
  has(key) {
    let exists = this.schema ? this.model.in(key) : this.model.has(key)
    return exists || (this.parent ? this.parent.has(key) : false )
  }
  set(key, obj) {
    return this.schema ? this.model.in(key).merge(obj) : this.model.set(key, obj)
  }
  get(key) {
    let exists = this.schema ? this.model.in(key) : this.model.has(key)
    if (exists) return this.model.get(key)
  ...
  resolve(key) {
    let match = this.schema ? this.schema.locate(key) : undefined
    if (!match) return key
  ...
  get cache() {
    ...
    let consumes = requires.concat(inputs).concat(this.contains).filter((v,i,a)=>a.indexOf(v)==i)
