...
class KineticObjectStream extends KineticEssence {
    ...
    this.core = new KineticEssence({
      transform(ko, enc, callback) {
        ...
        if (main.seen(ko)) { 
          ...
          // 2. ignore keys not part of consumes
          ...
          // tag data that can be intercepted by imported flow with main to prevent external propagation
          if (main.intercepts.has(ko.key)) {
            debug(main.identity, '!->', ko.key)
            main.mark(ko)
          } else {
            debug(main.identity, '-->', ko.key)
  ...
  get intercepts() {
    ...
    let keys = [].concat(...imports.map(x => Array.from(x.consumes)))
    return new Set(keys)
  }
