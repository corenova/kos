...
class KineticObjectStream extends KineticTransform {
  ...
  constructor(options={ requires: [], actions: [], subflows: [], buffer: false}) {
    super(options)
  ...
  use(transform) {
    ...
    if (transform instanceof KineticAction)       this._actions.add(transform)
    if (transform instanceof KineticObjectStream) this._subflows.add(transform)
    if (transform.join(this)) {
      this.inflow.pipe(transform).pipe(this.outflow)
    }
    return this
  }
